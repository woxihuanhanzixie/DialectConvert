...
class DialectPipelineEngine:
    ...
    def process_text(
        ...
        *,
        ...
        if enable_tts:
            ...
            if voice_match_requested and (teacher_result.get("wav_path") or qwen_voice_match_provider):
            ...
            voice_match_provider = (self.cfg.voice_conversion_provider or "").strip().lower()
            voice_match_input_mode = (
                "semantic_text_with_cloned_voice"
                if voice_match_provider in {"qwen_voice_clone", "qwen_vc", "qwen"}
                else "teacher_audio_to_audio"
            )
            voice_match_reason = (
    ...
    def process_audio(
        ...
        *,
        ...
        tts = None
        if enable_tts:
            baseline_text = pivot_text_zh or review["asr_reviewed_text"]
            baseline_input_mode = "pivot_text_zh" if pivot_text_zh else "review_text"
            old_voice = self.cfg.qwen_tts_voice
            if voice:
                self.cfg.qwen_tts_voice = voice
            active_voice = self.cfg.qwen_tts_voice
