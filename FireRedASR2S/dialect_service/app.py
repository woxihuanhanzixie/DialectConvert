...
ALLOWED_AUDIO_EXTS = {".wav", ".mp3", ".m4a", ".mp4", ".aac", ".amr", ".3gp", ".ogg", ".opus", ".webm", ".flac"}
    ...
    try:
        ...
        if file is not None and not speech_text:
            ...
            if asr_result is None:
                if not engine._local_asr_enabled(asr_cfg):
                    raise HTTPException(
                        status_code=502,
                        detail={
                            "error_code": "ASR_CLOUD_FAILED",
                            "message": "Cloud ASR failed before realtime synthesis.",
                            "reason": cloud_asr_error,
                        },
                    )
                asr_result = await asyncio.to_thread(engine._transcribe_with_local_fallback, wav_path, asr_cfg, enable_punc=enable_punc)
                asr_result["cloud_asr_error"] = cloud_asr_error
                warnings.append(f"cloud_asr_fallback: {cloud_asr_error}")
        ...
        if speaker_ref_audio is not None and voice_clone_enabled:
            try:
                ...
                reference_audio_validation = {
                    "frontend_mode": ref_frontend_mode,
