...
import argparse# 命令行参数解析（--wav_path, --model_dir )
import json
import logging
import os

from fireredasr2.asr import FireRedAsr2, FireRedAsr2Config  ##第二个参数表示选择的对应的模型是什么，aed或llm
from fireredasr2.utils.io import get_wav_info, write_textgrid #第一个get_wav_info表示1读取wav音频数据

logging.basicConfig(level=logging.INFO,
    format="%(asctime)s (%(module)s:%(lineno)d) %(levelname)s: %(message)s")  #表示日志
logger = logging.getLogger("fireredasr2.bin.speech2text")


parser = argparse.ArgumentParser()
parser.add_argument('--asr_type', type=str, required=True, choices=["aed", "llm"])  #必须 选择模型的参数
